        ...
        if (this.groupStudents.length > 1) {
            ...
            do {
                ...
                for (let i = 0; i < this.groupStudents.length - 1; i++) {
                    if (this.groupStudents[i].subjects[0] < this.groupStudents[i + 1].subjects[0]) {
...
function orderA() {
    matGroup.orderAscending();
    engGroup.orderAscending();
    geoGroup.orderAscending();
    litGroup.orderAscending();
    rearrangeGroup();
}

function orderD() {
    matGroup.orderDescending();
    engGroup.orderDescending();
    geoGroup.orderDescending();
    litGroup.orderDescending();
    rearrangeGroup();
}
...
function groupAvg(group, avgP, groupNum){
    if (group.groupStudents.length > 0){
        ...
        for (i=0; i<group.groupStudents.length; i++){
            tempGrade += group.groupStudents[i].subjects[groupNum];
            iterations ++;
        }
